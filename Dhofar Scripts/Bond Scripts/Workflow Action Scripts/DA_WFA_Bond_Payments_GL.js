			...
			try {
				...
				if(compoundable){
					...
					loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
						fieldId: 'debit',
						value: Number(expenseAmount2d)
					});
					try{
						loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
						fieldId: 'custcol_da_dr_3_decimal',
						value: Number(expenseAmount3d)
					});
					}
					catch (ex) {
				...
				log.error(ex.name, ex.message);
                  ...
                  loanPaymentRec.selectNewLine({
					...
					loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
						fieldId: 'account',
						value: scriptContext.newRecord.getValue('custrecord_da_bond_int_call_lia_acc')
					});
					log.debug('accuredAmount', accuredAmount);
					loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
				...
				log.error(ex.name, ex.message);
					...
					record.submitFields({
						...
						values: {
							'custrecord_da_bond_interest_trans_no': accuralJournalId
						}
					});
                  ...
                  try{
                   ...
                   try{
					...
					loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
						fieldId: 'debit',
						value: expenseAmount2d
					});
                  try{
                    loanPaymentRec.setCurrentSublistValue({
						sublistId: 'line',
						fieldId: 'custcol_da_dr_3_decimal',
						value: expenseAmount3d
					});
                  }catch(ex){
                    log.error(ex.name,ex.message);
                  }
