 ...
 */
function validateDuplicate_SaveRecord() {
	var bReturn = true;
	...
	if (!isEmpty(idRec) &&
			!isEmpty(idAgreement) &&
			!isEmpty(idEnt) &&
			checkHasDuplicateTLR(idRec, idAgreement, idEnt, idTierGroup, idItem, idItemClass)) {
		alert('Tier Level Reference record already exists for the combination of Rebate Agreement, Customer/Vendor, Tier Group, and Item/Item Classification');
		bReturn = false;
	}
