    ...
    function onRequest(context) {
        if (context.request.method === "GET") {
            var chartOptions = {
                ...
                colorAxis: { minValue: 0, /*colors: ['yellow', 'red']*/ }
            };
            var savedSearch = search.load({
                id: 'customsearch_da_subsidiary_profitability',
            }).run();
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var countryName = values["GROUP(subsidiary.country)"][0]["text"];
                var countryCode = values["GROUP(subsidiary.country)"][0]["value"];
            ...
            var form = ui.createForm({ title: 'Profit By Country', hideNavBar: true });
