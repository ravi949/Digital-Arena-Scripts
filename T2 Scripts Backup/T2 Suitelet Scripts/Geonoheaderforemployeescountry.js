    ...
    function onRequest(context) {
        if (context.request.method === "GET") {
            ...
            var savedSearch = search.load({
                id: 'customsearch_da_employees_country',
            }).run();
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var countryName = values["GROUP(custentity_da_emp_home_country)"][0]["text"];
                var countryCode = values["GROUP(custentity_da_emp_home_country)"][0]["value"];
                var count = Number(values["COUNT(formulanumeric)"]);
            ...
            var form = ui.createForm({ title: 'Employees by Home Country', hideNavBar: true });
            var chartField = form.addField({
                id: 'google_map_chart_html',
                label: 'Employees by Home Country',
