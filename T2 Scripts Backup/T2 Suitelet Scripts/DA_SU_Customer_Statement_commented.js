        ...
        function onRequest(context) {
            try {
                ...
                if (context.request.method === 'GET') {
                  ...
                  var pdcList = form.addSublist({
                        id: 'custpage_pdc_sublist',
                        type: ui.SublistType.LIST,
                        label: 'PDC',
                        tab: 'custpage_tab'
                    });
                  ...
                  pdcList.addField({
                        id: 'custpage_pdc_tran_date',
                        type: ui.FieldType.TEXT,
                        label: '<b>PDC</b><br/>Date'
                    });
                    pdcList.addField({
                        id: 'custpage_pdc_type',
                        type: ui.FieldType.TEXT,
                        label: 'Type'
                    });
                     pdcList.addField({
                        id: 'custpage_pdc_doc_no',
                        type: ui.FieldType.TEXT,
                        label: 'Document No'
                    });
                     pdcList.addField({
                        id: 'custpage_pdc_memo',
                        type: ui.FieldType.TEXT,
                        label: 'Memo'
                    });
                    pdcList.addField({
                        id: 'custpage_pdc_tran_status',
                        type: ui.FieldType.TEXT,
                        label: 'Status'
                    });
                    pdcList.addField({
                        id: 'custpage_pdc_amount',
                        type: ui.FieldType.CURRENCY,
                        label: 'Amount'
                    });
                     pdcList.addField({
                        id: 'custpage_pdc_remaining_amount',
                        type: ui.FieldType.CURRENCY,
                    ...
                    if (request.parameters.customerId) {
                          ...
                          subsidiaryField.defaultValue = subsidiaryId;
                            ...
                            if (request.parameters.customerId && request.parameters.endDate == "") {
                                transactionSearchObj = search.create({
                                    ...
                                    filters: [
                                        [
                                            ["name", "anyof", customerId], "AND", ["mainline", "is", "T"], "AND", ["type", "noneof", "SalesOrd", "Journal", "Estimate","RtnAuth","ItemShip","DepAppl","Check"],"AND",['subsidiary', 'anyof', subsidiaryId]
                                        ],
                                        "OR",
                                        [
                                ...
                                var opTransactionSearchObj = search.create({
                                    ...
                                    filters: [
                                        [
                                            ["name", "anyof", customerId], "AND", ["trandate", "within", firstDateString, formattedDateString], "AND", ["mainline", "is", "T"], "AND", ["type", "noneof", "SalesOrd", "Journal", "Estimate", "CustDep","RtnAuth","ItemShip","Check"],"AND",['subsidiary', 'anyof', subsidiaryId]
                                        ],
                                        "OR",
                                        [
                                ...
                                transactionSearchObj = search.create({
                                    ...
                                    filters: [
                                        [
                                            ...
                                            "AND",
                                            ["name", "anyof", customerId], "AND", ["mainline", "is", "T"], "AND", ["type", "noneof", "SalesOrd", "Journal", "Estimate","RtnAuth","ItemShip","DepAppl","Check"],
                      ...
                      var transactionSearchObj = search.create({
                           ...
                           filters:
                           [
                              ["type","anyof","Custom"+pdcRecID],
                              "AND",
                              ["mainline","is","T"],
                              "AND",
                              ["custbody_da_signatory_name","anyof",customerId],"AND",
                             ["custbody_da_remaining_amount_to_encash", "greaterthan", 0]
                           ],
                           columns:
                           [
                              ...
                              search.createColumn({name: "custbody_da_cheque_amount", label: "Cheque Amount"}),
                              search.createColumn({name: "custbody_da_remaining_amount_to_encash", label: "Remaining Amount To Encash"}),
                      ...
                      transactionSearchObj.filters.push(search.createFilter({
                        ...
                        log.debug("transactionSearchObj result count",searchResultCount);
                        transactionSearchObj.run().each(function(result){
                            var pdcdate = result.getValue({
                                    name: 'trandate'
                                });
                          var pdctype = result.getValue({
                                    name: 'type'
                                });
                          var pdctranid = result.getValue({
                                    name: 'tranid'
                                });
                          var pdcmemo = result.getValue({
                                    name: 'memomain'
                                });
                          var pdcstatus= result.getValue({
                                    name: 'statusref'
                                });
                          var pdcamount = result.getValue({
                                    name: 'custbody_da_cheque_amount'
                                });
                          var pdcramount = result.getValue({
                                    name: 'custbody_da_remaining_amount_to_encash'
                                });

                         if(pdcdate){
                            pdcList.setSublistValue({
                        id: 'custpage_pdc_tran_date',
                         line: k,
                         value:  pdcdate
                            });
                        ...
                        id: 'custpage_pdc_remaining_amount',
                         ...
                         value:-(pdcramount)
                          ...
                          pdcValue = parseFloat(pdcramount)+parseFloat(pdcValue);
                          k++;
                           return true;
                        });
                        endValue =parseFloat(openingBalance)-parseFloat(pdcValue);
                      pdcList.setSublistValue({
                        id: 'custpage_pdc_tran_date',
                         line: k,
                         value:'<b>PDC Value</b>'
...
'<thead> <tr> <th colspan=\"3\">Date</th> <th colspan=\"4\">Type</th> <th colspan=\"5\">Doc No</th> <th colspan=\"3\">Status</th> <th colspan=\"8\">Memo</th> <th align=\"right\" colspan=\"4\">Amount</th> <th align=\"right\" colspan=\"4\">Remaining Amount</th> </tr> </thead>';
                  for (var k = 0; k< numLines1; k++) {
                        var pdcDate = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_tran_date',
                            line: k
                        });
                        var pdcType = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_type',
                            line: k
                        });
                        var pdcDocNo = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_doc_no',
                            line: k
                        });
                        var pdcStatus = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_tran_status',
                            line: k
                        });
                     var pdcMemo = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_memo',
                            line: k
                        });
                        var pdcAmount = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_amount',
                            line: k
                        });
                        var pdcrAmount = request.getSublistValue({
                            group: 'custpage_pdc_sublist',
                            name: 'custpage_pdc_remaining_amount',
                            line: k
                        });

                    xml +=' <tr>'+
                                '<td colspan="3">' + ( pdcDate ? ( pdcDate) : '') + '</td>'+
                                '<td colspan="4">' + (pdcType ? pdcType : '') + '</td>'+
                                '<td colspan="5">' + (pdcDocNo ? pdcDocNo : '') + '</td>'+
                                '<td colspan="3">' + (pdcStatus ? pdcStatus : '') + '</td>'+
                                '<td colspan="8">' + (pdcMemo  ? pdcMemo  : '') + '</td>'+
                                '<td align="right" colspan="4">' + (pdcAmount  ? pdcAmount  : '') + '</td>'+
                                '<td align="right" colspan="4">' + (pdcrAmount ? pdcrAmount: '') + '</td>'+
                ...
                form.clientScriptModulePath = './DA_CS_Attach_Customer_Statement_comment.js';
            } catch (ex) {
