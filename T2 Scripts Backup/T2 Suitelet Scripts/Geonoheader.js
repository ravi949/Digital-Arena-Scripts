    ...
    function onRequest(context) {
        if (context.request.method === "GET") {
            ...
            var savedSearch = search.load({
                id: 'customsearch_da_students_by_country_of_b',
            }).run();
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var count = Number(values["COUNT(internalid)"]);
            ...
            var form = ui.createForm({ title: 'Students by Country of Birth', hideNavBar: true });
            var chartField = form.addField({
                id: 'google_map_chart_html',
                label: 'Students by Country of Birth',
