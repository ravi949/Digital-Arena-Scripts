	...
	function(search, record, format) {
		...
		function onAction(scriptContext) {
			try {
				...
				var loanIssuanceRef = scriptContext.newRecord.getValue('custrecord_da_bond_settlement_ref');
				if(loanIssuanceRef){
					record.delete({
						type:'customtransaction_da_bond_settlement',
						id : loanIssuanceRef
					})
				}
				...
				customrecord_da_loan_settingsSearchObj.run().each(function(result) {
					...
					var date = scriptContext.newRecord.getValue('custrecord_da_maturity_date');
					log.debug('date', date);
				...
				log.error(ex.name, ex.message);
					...
					scriptContext.newRecord.setValue('custrecord_da_bond_settlement_ref', accuralJournalId);
					//return true;
				});
			} catch (ex) {
				log.error(ex.name, ex.message);
