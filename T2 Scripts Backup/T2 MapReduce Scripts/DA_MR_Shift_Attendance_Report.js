		...
		function(search, record, runtime, format) {
				...
				try {
					...
					var customrecord_da_shift_attendance_reportSearchObj = search.create({
					   ...
					   [
					      ...
					      search.createColumn({name: "custrecord_da_atten_report_date", label: "Date"})
					   ]
					});
			...
			function reduce(context) {
				try {
					  ...
					  attendanceReportRec.setValue('custrecord_da_atten_report_date', new Date());
					  ...
					  var customrecord_da_shift_time_sheetSearchObj = search.create({
						   ...
						   [
						      ...
						      "AND", 
						      ["custrecord_da_time_sheet_date","within","today"],
                              "AND",
     					      ["custrecord_da_time_sheet_shift_type","anyof","1","3"]
						   ],
						   columns:
						   [
						      ...
						      search.createColumn({name: "custrecord_da_time_sheet_employee", label: "Employee"}),
						      search.createColumn({name: "custrecord_da_time_sheet_date", label: "Date"}),
						      search.createColumn({name: "custrecord_da_time_sheet_type", label: "Type"}),
						      search.createColumn({name: "custrecord_da_time_sheet_shift", label: "Shift"}),
						      search.createColumn({name: "custrecord_da_time_sheet_shift_in", label: "Shift Time In"}),
						      search.createColumn({name: "custrecord_da_ts_shift_time_out", label: "Shift Time Out"}),
						      search.createColumn({name: "custrecord_da_time_sheet_shift_type", label: "Shift Type"}),
						      search.createColumn({name: "custrecord_da_time_sheet_subsidairy", label: "Subsidiary"}),
						      search.createColumn({name: "custrecord_da_emp_working_shift", label: "Working In"})
						   ]
						});
						var searchResultCount = customrecord_da_shift_time_sheetSearchObj.runPaged().count;
						log.debug("customrecord_da_shift_time_sheetSearchObj result count",searchResultCount);
					  ...
					  var customrecord_da_shift_attendnaceSearchObj = search.create({
					   ...
					   filters:
					   [
					      ["custrecord_da_shift_attendance_date","within","today"], 
					      "AND", 
					   ...
					   [
					      ...
					      search.createColumn({name: "custrecord_da_shift_attendance_employee", label: "Employee"}),
					      search.createColumn({name: "custrecord_da_shift_att_emp_subsidary", label: "Subsidiary"}),
					      search.createColumn({name: "custrecord_da_shift_attendance_date", label: "Date"}),
					      search.createColumn({name: "custrecord_da_attenance_in_out", label: "In/Out"}),
					      search.createColumn({
					         name: "custrecord_da_shift_attendance_time_in",
					         sort: search.Sort.ASC,
					         label: "Time"
					      }),
					      search.createColumn({name: "custrecord_da_shift_att_emp_location", label: "Location"})
					   ]
					});
					...
					var customrecord_da_holiday_datesSearchObj = search.create({
					   ...
					   filters:
					   [
					      ["custrecord_holiday_date","within","today"], 
					      "AND", 
					...
					customrecord_da_holiday_datesSearchObj.run().each(function(result){
					   ...
					   attendanceReportRec.setValue('custrecord_da_atten_report_holiday_code', result.getValue({name:'name', join:'CUSTRECORD_DA_HOLIDAY_PARENT'}))
					   return true;
					});
					...
					var customrecord_da_permissionSearchObj = search.create({
					   ...
					   filters:
					   [
					      ["custrecord_da_permission_date","within","today"], 
					      "AND", 
					      ["custrecord_da_permission_employee","anyof",empId]
					   ],
					   columns:
					   [
					      search.createColumn({name: "custrecord_da_permission_employee", label: "Employee"}),
					      search.createColumn({name: "custrecord_da_permission_type", label: "Permission Type"}),
					      search.createColumn({name: "custrecord_da_permission_date", label: "Date"}),
					      search.createColumn({name: "custrecord_da_permission_from_time", label: "From Time"}),
					      search.createColumn({name: "custrecord_da_permission_to_time", label: "To Time"}),
					      search.createColumn({name: "custrecord_da_permission_total_time", label: "Total Time"}),
					      search.createColumn({name: "custrecord_da_permission_approval_status", label: "Approval Status"})
					   ]
					});
					...
					var totalPermissionHrs = 0; var permissionCode = ' ';
					...
					var customrecord_da_leavesSearchObj = search.create({
					   ...
					   filters:
					   [
					      [[["formulanumeric: {today}-{custrecord_da_leave_start_date}","greaterthanorequalto","0"],"AND",["formulanumeric: {custrecord_da_leave_end_date}-{today}","greaterthanorequalto","0"]],"OR",[["custrecord_da_leave_start_date","within","today"]],"OR",[["custrecord_da_leave_end_date","within","today"]]], 
					      "AND", 
					   ...
					   columns:
					   [
					      search.createColumn({name: "custrecord_da_leave_employee", label: "Employee"}),
					      search.createColumn({name: "created", label: "Request Date"}),
					      search.createColumn({name: "custrecord_da_leave_type", label: "Leave Type"}),
					      search.createColumn({name: "custrecord_da_leave_start_date", label: "Leave Start Date"}),
					      search.createColumn({name: "custrecord_da_leave_end_date", label: "Leave End Date"}),
					      search.createColumn({name: "custrecord_da_leave_aproval_status", label: "Approval Status"})
					   ]
					});
