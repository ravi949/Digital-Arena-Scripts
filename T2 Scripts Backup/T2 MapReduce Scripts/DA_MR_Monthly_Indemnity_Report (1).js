...
define(['N/search', 'N/record', 'N/runtime', 'N/format'],
   ...
   [
      ...
      search.createColumn({name: "custrecord_da_ind_type", label: "Type"}),
      search.createColumn({name: "custrecord_da_indemnity_month", label: "Month"}),
      search.createColumn({name: "custrecord_da_ind_employee", label: "Employee"}),
      search.createColumn({name: "custrecord_da_ind_emp_subsidairy", label: "Subsidiary"}),
      search.createColumn({name: "custrecord_da_ind_emp_hire_date", label: "Hire Date"}),
      search.createColumn({name: "custrecord_da_ind_working_years", label: "Working Years"}),
      search.createColumn({name: "custrecord_da_salary_eligible_for_indem", label: "Salary Eligible for Indemnity"}),
      search.createColumn({name: "custrecord_indemnity_days", label: "Indemnity Days"}),
      search.createColumn({name: "custrecord_da_ind_opening_balance", label: "Opening Balance"}),
      search.createColumn({name: "custrecord_da_ind_adding_amount", label: "Additional Amount"}),
      search.createColumn({name: "custrecord_da_ind_final_amount", label: "Final Amount"})
   ]
});
  ...
  record.delete({
   ...
   return true;
                ...
                var postingPeriod = scriptObjPeriod.getParameter({
                    name: 'custscript_da_wfa_indemnity_po'
                });
                log.debug('postingPeriod', postingPeriod);
                var customrecord_da_indemnity_reportSearchObj = search.create({
                    type: "customrecord_da_indemnity_report",
                    filters: [
                        'custrecord_da_indemnity_month', 'anyof', postingPeriod
                    ]
                });
                var searchCount = customrecord_da_indemnity_reportSearchObj.runPaged().count;
                log.debug("customrecord_da_indemnity_reportSearchObj count", searchCount);
                customrecord_da_indemnity_reportSearchObj.run().each(function(result) {
                    record.delete({
                        type: 'customrecord_da_indemnity_report',
                        id: result.id
        ...
        function reduce(context) {
            try {
                ...
                if (searchResultCount == 0) {
                    ...
                    var customrecord_da_general_settingsSearchObj = search.create({
                        ...
                        columns: [
                            ...
                            search.createColumn({
                                name: "custrecord_da_working_days_per_year",
                ...
                var dateObj = scriptObjDate.getParameter({
                    name: 'custscript_da_wfa_indemnity_da'
                });
                ...
                log.debug('parsedDate',parsedDate);
                 ...
                 // log.debug('da_today',da_today);
                   ...
                   // var secondDate = da_today;
                    var secondDate = parsedDate;
                    log.debug('secondDate',secondDate);
            ...
            try {
              var scriptObj = runtime.getCurrentScript();
                var recordId = scriptObj.getParameter({
                    name: 'custscript_da_indemnity_report_rec_id'
                });
                log.debug('recordId', recordId);
                var processRec = record.submitFields({
                    type: 'customrecord_da_indemnity_report_postdat',
                    id: recordId,
                    values: {
                        'custrecord_da_generate_report': false
                    }
                });
