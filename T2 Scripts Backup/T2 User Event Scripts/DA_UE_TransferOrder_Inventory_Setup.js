    ...
    function(message, serverWidget, search, runtime, record) {
        ...
        function afterSubmit(scriptContext) {
            try {
              ...
              var location = transferOrderrec.getValue({
                        fieldId: 'location'
                    });
              
                var numLines = transferOrderrec.getLineCount({
                    sublistId: 'item'
                });
                //log.debug('numLines', numLines);
                for (var i = 0; i < numLines; i++) {
                    var item = transferOrderrec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'item',
                        line: i
                    });
                    log.debug('item', item);
                    var quantity = transferOrderrec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'quantity',
                        line: i
                    });
                    var availbleqty = transferOrderrec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'quantityavailable',
                        line: i
                    });
                   /* var location = transferOrderrec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'location',
                        line: i
                    });*/
                   // log.debug('location', location);
                    ...
                    var inventoryDetailSet = transferOrderrec.getSublistValue({
                        sublistId: 'item',
                        ...
                        if (lookup.islotitem == true) {
                           // log.debug('lotItem');
                            ...
                            if (serialCount > 0) {
                                if (quantity > 0) {
                                    if (availbleqty > 0) {
                                        var lineNum = transferOrderrec.selectLine({
                                            sublistId: 'item',
                                            line: i
                                        });
                                        var subrec = transferOrderrec.getCurrentSublistSubrecord({
